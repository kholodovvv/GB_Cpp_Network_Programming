...
int main(int argc, const char * const argv[]) {
    ...
    if (2 == argc) {
        ...
        //Файл для сохранения полученных данных (пакетов) в файл формата pcap
        pcap_dumper_t *dumpfp = pcap_dump_open(handle, "./capture_packet");

        //pcap_loop(handle, num_packets, pcap_callback, nullptr);
        pcap_loop(handle, num_packets, pcap_callback, (unsigned char *) dumpfp);
