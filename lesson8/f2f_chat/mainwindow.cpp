...
void MainWindow::on_pbtnExitChat_clicked()
{
    ...
    mychat->SendMessage(1, "bye", mychat->GetIdProgram(), &address); //оповещение всех пользователей чата о выходе
...
void MainWindow::on_pbtnSendMessage_clicked()
{
    if(ui->lneEnterMessage->text().isEmpty()){
        ...
        if(-1 == lstIndex){
            QMessageBox::critical(this, "Ошибка!", "Для отправки личного сообщения, нужно выбрать адресата!");
            return;
        }
...
void MainWindow::on_pbtnMailing_clicked()
{
    ...
    if(ui->lneEnterMessage->text().isEmpty()){
        ...
        mychat->SendMessage(2, ui->lneEnterMessage->text(), mychat->GetIdProgram(), &address);
